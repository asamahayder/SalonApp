...
class GetServicesBySalonIdUseCase @Inject constructor(
    private val repository: ServicesRepository,
){
    operator fun invoke(salonId:Int): Flow<Resource<List<Service>>> = flow {
        try {

            emit(Resource.Loading())

            val response = repository.getServicesBySalonId(salonId)
