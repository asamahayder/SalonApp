...
class GetRequestsBySalonIdUseCase @Inject constructor(
    private val repository: RequestRepository,
){
    operator fun invoke(salonId:Int): Flow<Resource<List<Request>>> = flow {
        try {

            emit(Resource.Loading())

            val response = repository.getRequestsBySalonId(salonId)
