...
class UpdateOpeningHoursUseCase @Inject constructor(
    private val repository: OpeningHoursRepository,
){
    operator fun invoke(openingHours: OpeningHours): Flow<Resource<OpeningHours>> = flow {
        try {
            emit(Resource.Loading())

            val response = repository.updateOpeningHours(openingHours = openingHours)
