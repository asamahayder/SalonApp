...
class DenyRequestUseCase @Inject constructor(
    private val repository: RequestRepository,
){
    operator fun invoke(requestId:Int): Flow<Resource<List<Request>>> = flow {
        try {

            emit(Resource.Loading())

            val response = repository.denyRequest(requestId)
