...
class GetDefaultOpeningHoursUseCase @Inject constructor(
    private val repository: OpeningHoursRepository,
){
    operator fun invoke(employeeId:Int): Flow<Resource<OpeningHours>> = flow {
        try {
            emit(Resource.Loading())

            val response = repository.getNormalHours(employeeId)
