...
class RegisterUseCase @Inject constructor(
    ...
    operator fun invoke(user: UserRegister): Flow<Resource<String>> = flow {
        try {
            ...
            when(user.role){
                Constants.ROLE_OWNER -> message = repository.registerOwner(user).message
                Constants.ROLE_EMPLOYEE -> message = repository.registerEmployee(user).message
                Constants.ROLE_CUSTOMER -> message = repository.registerCustomer(user).message
                else -> {
